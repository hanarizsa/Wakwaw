...
function createBirthdayContent() {
    ...
    const originalMessage = `Ada hari dimana dunia terasa seperti beban yang tidak kamu pilih, ada saat dimana hidup terasa lebih gelap daripada yang kamu bayangkan namun kamu tetap disini masih mencoba untuk melangkah meski pelan meski ragu dan bahumu mulai terasa lelah. Aku ingin menuliskan sesuatu yang hanya bisa ditujukan padamu. Bukan ucapan manis bukan juga kata-kata yang diulang ribuan kali oleh banyak orang. Aku ingin menuliskan tentang keberanianmu yang tidak pernah kamu sadari. Tentang caramu bangkit ketika tidak ada yang tahu kamu jatuh, tentang caramu tersenyum pada dunia yang tidak selalu tersenyum padamu.

Aku hanya meminta hal sederhana namun sangat berarti. Aku berharap kamu diberi ruang untuk bernafas tanpa rasa bersalah, aku berharap kamu diberi pelukan yang tidak menuntut apa pun. dan kalo boleh aku ingin menjadi bagian kecil dari hidupmu yang membuat hidupmu tidak seberat kemarin. seseorang yang mendengarkan bahkan saat kamu tidak tahu harus bicara apa, seseorang yang diamnya bisa kamu percaya sama seperti aku mempercayakan hatiku kepadamu.

Happy Birthday Mas! Setiap tahunnya pasti akan selalu ada rintangan baru, tapi aku berharap semoga kamu selalu menemukan alasan untuk tetap bertahan dan menjadikan aku rumah yang teduh dan selalu memberi kehangatan setiap kamu pulang.`;

    const encryptedMessage = caesarEncode(originalMessage, 15);
    ...
    return `
        <div class="content-display birthday-content">
            <h1>🎂 Happy Birthday, Mas! 💕</h1>
            <div class="birthday-flower-container" style="position: relative;">
                <div class="birthday-flower">🌹</div>
            </div>
            <div class="birthday-message">
                <p style="text-align: center; margin-bottom: 20px; color: #7f8c8d; font-style: italic;">
                    Aku punya pesan spesial untukmu, tapi kamu harus decode dulu ya! 😊
                </p>

                <div class="cipher-container" style="
                    background: linear-gradient(135deg, #fef5f8 0%, #fff5f9 100%);
                    border: 2px dashed var(--primary-color);
                    border-radius: 15px;
                    padding: 25px;
                ...
                <div id="birthdayInteraction" style="margin-top: 30px;">
                    <div style="text-align: center;">
                        <p style="margin-bottom: 15px; color: #555; font-size: 1.05rem;">
                            Masukkan kata kunci untuk decode pesan:
                        </p>
                        <input type="text" id="birthdayInput" placeholder="Masukkan kata kunci..."
                            style="
                                width: 100%;
                                max-width: 400px;
                                padding: 15px;
                            ...
                            ">
                        <br>
                        <button onclick="checkBirthdayAnswer()" class="btn-primary" style="margin-top: 10px;">
                            Submit
                        </button>
                        <div id="birthdayFeedback" style="margin-top: 20px; font-size: 1rem;"></div>
                    </div>
                </div>
            </div>
        </div>
...
function checkBirthdayAnswer() {
    birthdayAttempts++;
    const feedback = document.getElementById('birthdayFeedback');
    const input = document.getElementById('birthdayInput');

    if (birthdayAttempts < 2) {
        feedback.innerHTML = `
            <p style="color: #e74c3c; font-weight: 600;">
                Hmm, belum tepat nih! Coba lagi ya! 💕 (Attempt ${birthdayAttempts}/2)
            </p>
        `;
        input.value = '';
        input.style.animation = 'shake 0.5s';
    ...
    } else if (birthdayAttempts === 2) {
        feedback.innerHTML = `
            <p style="color: #e74c3c; font-weight: 600; margin-bottom: 15px;">
                Waduh, salah lagi! 😅
            </p>
            <button onclick="showFunnyQuestion()" class="btn-secondary" style="margin: 10px 5px;">
                Iseng dulu ah 😏
            </button>
        `;
        input.disabled = true;
...
const funnyQuestions = [
    {
        question: "Kalau aku jadi WiFi, password-nya apa?",
        answer: "Love you forever! 💕"
    },
    {
        question: "Kenapa aku ga pernah nyasar?",
        answer: "Karena kamu selalu jadi arah pulang aku 🏠"
    },
    {
        question: "Aku ini mirip bank ga sih?",
        answer: "Iya, soalnya aku selalu simpan semua momen kita di hati! 💝"
    },
    {
        question: "Kamu tahu ga kenapa aku suka sama angka 143?",
        answer: "Karena 1 4 3 = I Love You! 💕"
    },
    {
        question: "Kalau perasaan aku ini makanan, kira-kira apa ya?",
        answer: "Mochi! Manis, lembut, dan bikin nagih! 🍡"
    }
];

function showFunnyQuestion() {
    const feedback = document.getElementById('birthdayFeedback');

    if (funnyClickCount < funnyQuestions.length) {
        const q = funnyQuestions[funnyClickCount];
        feedback.innerHTML = `
            <div style="
                background: #fff9fc;
                border: 2px solid var(--primary-color);
            ...
            ">
                <p style="color: var(--primary-color); font-weight: 600; font-size: 1.1rem; margin-bottom: 15px;">
                    ${q.question}
                </p>
            ...
            ${funnyClickCount < funnyQuestions.length - 1
                ? '<button onclick="showFunnyQuestion()" class="btn-secondary" style="margin: 10px 5px;">Lagi dong! 😆</button>'
                : '<button onclick="showGiveUpOption()" class="btn-primary" style="margin: 10px 5px;">Udah capek mikir... 😩</button>'
            }
        `;
        funnyClickCount++;
    }
    ...
    feedback.innerHTML = `
        ...
        ">
            <p style="color: #555; font-size: 1.1rem; margin-bottom: 20px;">
                Udah capek ya mas? Hehe, oke deh... 😊
            </p>
            <button onclick="revealBirthdayMessage()" class="btn-primary">
                Menyerah! Tunjukkan Pesannya! 💕
            </button>
        </div>
    `;
}
...
Happy Birthday Mas! Setiap tahunnya pasti akan selalu ada rintangan baru, tapi aku berharap semoga kamu selalu menemukan alasan untuk tetap bertahan dan menjadikan aku rumah yang teduh dan selalu memberi kehangatan setiap kamu pulang.`;
    ...
    const interaction = document.getElementById('birthdayInteraction');
    interaction.innerHTML = `
        ...
        ">
            <div style="text-align: center; margin-bottom: 25px;">
                ...
                <h3 style="color: var(--primary-color); font-size: 1.5rem; margin-bottom: 10px;">
                    Pesan Decoded! 🎉
                </h3>
                <p style="color: #7f8c8d; font-style: italic; font-size: 0.95rem;">
                    Ini adalah pesan asli untukmu, Mas...
                </p>
            </div>
